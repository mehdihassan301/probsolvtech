      ...
      try {
        const response = await fetch('https://api.web3forms.com/submit', {
          ...
          body: JSON.stringify({ 
            ...
            ...formState, 
            subject: `Budget Support Request - ${formState.name}`,
            from_name: 'ProbSolv Pricing Support'
          }),
        });
        if (response.ok) {
          setSubmitted(true);
      ...
      try {
        const response = await fetch('https://api.web3forms.com/submit', {
          ...
          body: JSON.stringify({ 
            access_key: "7eeb8a38-00ac-4282-a1b1-58bde34a471c",
            ...formState, 
            subject: `Collaboration Proposal - ${formState.name}`,
            from_name: 'ProbSolv Collab Form'
          }),
        });
        if (response.ok) {
          setSubmitted(true);
        ...
        try {
            const response = await fetch('https://api.web3forms.com/submit', {
                ...
                body: JSON.stringify({ 
                  access_key: "7eeb8a38-00ac-4282-a1b1-58bde34a471c",
                  ...formState, 
                  subject: `Bespoke Solution Request - ${formState.name}`,
                  from_name: 'ProbSolv Custom Pricing'
                }),
            });
            if (response.ok) {
                setSubmitted(true);
              ...
              <div className="relative z-10 flex flex-col flex-grow">
                ...
                <div className="mt-4 min-h-[120px]">
                  {tier.price.toLowerCase().includes('contact') ? (
                    <p className="font-heading text-4xl sm:text-5xl font-extrabold text-text_light dark:text-text_dark">{tier.price}</p>
                  ) : tier.originalPrice ? (
                    <>
                      <div className="flex items-baseline gap-x-2">
                        ...
                        <p className={`font-heading font-extrabold text-text_light dark:text-text_dark ${currency === 'PKR' ? 'text-3xl sm:text-4xl' : 'text-4xl sm:text-5xl'}`}>
                          {displayPrice}
                        </p>
                      </div>
                      {tier.discount && (
