...
const ProjectBriefPage: React.FC<{ plan?: string }> = ({ plan }) => {
  const router = useRouter();
  const planName = plan || 'General Inquiry';
  const selectedPlan = pricingTiers.find(p => p.name === planName);
  const planBudget = selectedPlan ? selectedPlan.price : 'Custom';
    ...
    if (validate()) {
      ...
      try {
        const submissionData = {
          ...
          subject: `New Project Brief - ${formState.name}`,
          from_name: 'ProbSolv Project Brief'
        };
