...
import { serviceDetails, blogPosts } from '../components/constants';
const ServiceDetailPage: React.FC<{ id: string }> = ({ id }) => {
  const router = useRouter();
  const service = serviceDetails.find(s => s.id === id);

  // Redirect or Show 404 if service not found
  useEffect(() => {
    if (!service) {
                          ...
                          {relatedPosts.map((post) => (
                              ...
                              >
                                  <div className="aspect-w-16 aspect-h-9 overflow-hidden rounded-t-xl">
                                      <img src={post.image} alt={post.title} loading="lazy" className="w-full h-48 object-cover transition-transform duration-500 group-hover:scale-110" />
                                  </div>
