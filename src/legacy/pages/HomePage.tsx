    ...
    <>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {featuredServices.map((service) => (
              <button key={service.id} onClick={() => router.push(`/services/${service.id}`)} className="w-full h-full text-left relative group bg-surface_light dark:bg-surface_dark border border-border_light dark:border-border_dark rounded-xl p-8 transition-all duration-300 hover:shadow-2xl hover:shadow-primary/20 dark:hover:shadow-accent/20 hover:-translate-y-2 hover:scale-105 hover:border-primary dark:hover:border-accent flex flex-col items-start overflow-hidden">
                <AnimatedCardBackground />
          ...
          <button onClick={() => router.push('/services')} className="mt-12 px-5 sm:px-6 py-2.5 bg-primary text-white font-semibold rounded-lg text-sm hover:bg-purple-600 transition-all duration-300 transform hover:scale-105">
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-8 max-w-5xl mx-auto">
                ...
                <div className="w-full p-8 bg-surface_light dark:bg-surface_dark border border-border_light dark:border-border_dark rounded-xl text-left transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
                    ...
                    <ul className="space-y-4 text-subtext_light dark:text-subtext_dark">
                        ...
                        <li className="flex items-start gap-3">
                            <span className="text-red-500 mt-1 font-bold">✕</span>
                            <span><strong>The "Template" Identity:</strong> A generic website that looks like every other competitor and fails to build trust.</span>
                        </li>
                        <li className="flex items-start gap-3">
                            <span className="text-red-500 mt-1 font-bold">✕</span>
                            <span><strong>The Revenue Ceiling:</strong> Unable to take on more clients because you are trading time for money.</span>
                        </li>
                    </ul>
                </div>
                {/* After */}
      ...
      <AnimatedSection className="py-16 sm:py-20 md:py-24 bg-bg_light dark:bg-surface_dark">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
              ...
              <div key={item.id} className="group relative overflow-hidden rounded-xl aspect-w-4 aspect-h-3 shadow-lg transition-all duration-300 hover:shadow-2xl hover:shadow-primary/20 dark:hover:shadow-accent/20 hover:-translate-y-2 hover:scale-105">
                ...
                <div className="absolute inset-0 flex flex-col justify-end p-6 text-white">
                  ...
                  <div className="mt-4 opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300">
                    <button onClick={() => router.push(`/portfolio/${item.id}`)} className="font-semibold text-white bg-primary/80 backdrop-blur-sm px-4 py-2 rounded-md hover:bg-primary">
                      View Case Study
                    </button>
                  </div>
                </div>
          ...
          <button onClick={() => router.push('/portfolio')} className="mt-12 px-5 sm:px-6 py-2.5 bg-primary text-white font-semibold rounded-lg text-sm hover:bg-purple-600 transition-all duration-300 transform hover:scale-105">
            View Full Portfolio
          </button>
        </div>
      </AnimatedSection>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="relative max-w-4xl mx-auto text-center bg-gradient-to-r from-primary to-purple-600 p-8 sm:p-12 rounded-xl shadow-2xl shadow-primary/30 overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <button onClick={() => router.push('/contact')} className="mt-8 px-5 py-2.5 text-sm sm:px-6 sm:py-3 sm:text-base bg-white text-primary font-semibold rounded-lg hover:bg-gray-200 transition-all duration-300 transform hover:scale-105">
                        Get Tranformation
                    </button>
                </div>
            </div>
